...
def test_results_view_post_valid_calls_service_with_username(client, mocker):
    ...
    mock_service = mocker.patch(
        'apps.search.views.SherlockService.search', return_value=[]
    )
...
def test_results_view_renders_hits(client, mocker):
    ...
    fake_results = [
        SiteResult(
            site_name='GitHub', url='https://github.com/torvalds',
            status='found', response_time_ms=100, error_message=None,
        ),
        SiteResult(
            site_name='Reddit', url='https://reddit.com/user/torvalds',
            status='not_found', response_time_ms=50, error_message=None,
        ),
    ]
...
def test_results_view_renders_error_on_timeout(client, mocker):
    ...
    mocker.patch(
        'apps.search.views.SherlockService.search',
        side_effect=ServiceTimeoutError("Timeout"),
    )
